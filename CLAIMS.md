...
| A prior has to beat its consequent's own base rate, and it is the lower bound of its rate that must clear the line, so a pair resting on three people cannot outrank one resting on three hundred; measured against a known latent structure in 19,719 real respondents | `benchmarks/external/`, `server/tests_prior_lift.py` |
