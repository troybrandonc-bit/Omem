...
| The commons can be asked a question rather than only downloaded whole, every answer carries the people and installations it rests on, and a question it cannot support is refused with the reason instead of answered with an empty list | `server/tests_commons_ask.py` |
| An agent can ask one question at the moment it is deciding and get an answer carrying the people behind it, with what this install saw itself ranked above what it borrowed, answered from disk so it works with the commons unreachable or never contacted | `server/tests_ask.py` |
| A belief can be weighed against the population without being ruled on: the evidence pointing toward it and away from it, each with its counts, and never a verdict of true or false, because deciding what is true about a person from statistics about other people is the thing this refuses | `server/tests_ask.py` |
...
| A generator's record says how much better than the claim's own base rate it has been, not merely how often it was right, because a guesser that only predicts common things has a perfect record and carries no information | `server/tests_self_audit.py` |
| Every rate OMEM reports about itself carries the count it rests on, and says so when that count is too small to be a rate, when its lift is unknown, or when it is no better than chance | `server/tests_self_audit.py` |
| A leap pass reports the mean base rate of the claims it chose to speak about beside the mean over every claim available, so a pass that answers only the easy ones cannot look like one that answers the hard ones | `server/tests_self_audit.py` |
